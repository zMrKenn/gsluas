...
local master_switch = ui_new_checkbox("LUA", "A", "Enable sigma adaptive wpn")
...
for i=1, #config_names do
    rage[i] = {
        ...
        accuracy_boost = ui_new_combobox("LUA", "A", "[" .. config_names[i] .. "] Accuracy boost", {"Low", "Medium", "High", "Maximum"}),
...
--local ref_doubletap_mode = ui_reference("RAGE", "Other", "Double tap")
local ref_doubletap_stop = ui_reference("RAGE", "Aimbot", "Double tap quick stop", { "Slow motion", "Duck", "Move between shots" })
--
local in_speed
...
local function set_config(idx)
    ...
    if (ui_get(rage[i].ns_hitchance)) == 1 then
        nshitchance = (ui_get(rage[i].hitchance))
    else 
        nshitchance = (ui_get(rage[i].ns_hitchance))
    end

    if (ui_get(rage[i].hitchance_ovr)) == 1 then
        hitchanceovr = (ui_get(rage[i].hitchance))
    else
        hitchanceovr = (ui_get(rage[i].hitchance_ovr))
    end

    local hc_val =  (ui_get(ovr_hc_key) and (hitchanceovr)) or                              -- OVERRIDE HC
                    ((ui_get(rage[i].air_hc_enable) and not onground) and ui_get(rage[i].hitchance_air)) or -- IN AIR HC
                    (is_scoped == 0 and contains(scoped_wpn_idx, i) and (nshitchance)) or -- NO SCOPE HC ON SCOPED WPNS
